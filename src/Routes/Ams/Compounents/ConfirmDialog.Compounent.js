...
import {fetchRemoveAm} from "../../../redux/actions/actions";

export default function ConfrimDialog({isOpen , setOpen, amEmail}) {

    const dispatch = useDispatch();
    const submitRemove = ()=>{
        dispatch(fetchRemoveAm(amEmail))
            .then(()=>{
                    setTimeout(()=>{
            ...
            <Dialog className="p-2" open={isOpen} onClose={handleClose}>
                ...
                <DialogContent>
                    <DialogContentText style={{fontSize : "1.4rem"}} className="lora-400 gacela-black21 mb-lg-3">
                        Voulez-vous vraiment supprimer ce Compte : {amEmail} ?
                    </DialogContentText>
