...
export default function ConfrimDialog({isOpen , setOpen}) {
    ...
    const submitUser = ()=>{
        ...
        dispatch(fetchAddDecideur(signUpForm))
            .then(()=>{
                    setTimeout(()=>{
    ...
    return (
        <div>
            <Dialog className="p-2" open={isOpen} onClose={handleClose}>
                <DialogTitle style={{fontSize : "2rem"}} className="lora-700 gacela-orange">Nouveau compte de décideur</DialogTitle>
                <DialogContent>
                    <DialogContentText >
                        ...
                        <div className="w-100 mt-lg-3 mt-lg-3">
                            <Button onClick={()=>submitUser()} style={{color : "white"}} disabled={!(signUpForm.name?.length && signUpForm.family_name?.length && signUpForm.phone_number?.length && signUpForm.email?.length && signUpForm.password?.length && signUpForm.confirmPassword == signUpForm.password ) } id="signUpButton" className="w-100" variant="contained">Ajouter le profile</Button>
                        </div>
                    </DialogContentText>
                </DialogContent>
            </Dialog>
