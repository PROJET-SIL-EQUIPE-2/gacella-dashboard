...
export default function DecideursProfilesGestionRoute() {
  ...
  const columns = [
      ...
      render: (rowData) => (
        <div className="d-flex align-items-center">
          ...
          <div className="pl-2 flex-column">
            <div className="roboto-500 gacela-black21">
              {" "}
              {rowData.familyName} {rowData.name}{" "}
            </div>
          </div>
        </div>
      ),
    ...
    },
    {
      ...
      field: "phoneNumber",
      render: (rowData) => (
        <div className="roboto-500">{rowData.phone_number}</div>
      ),
    },
    {
      title: "Bloqué ?",
      field: "blocked",
      render: (rowData) => (
        <BlockProfileToggleButton
          email={rowData.email}
          isBlocked={rowData.blocked}
        />
      ),
    },
  ];
  ...
  const actions = [
    {
      icon: () => (
        <img style={{ height: "40px", width: "40px" }} src={AddIcon} />
      ),
      tooltip: "Ajouter un décideur",
      position: "toolbar",
      isFreeAction: true,
      onClick: (event) => {
        setSignUpDialogOpenStatus(true);
      },
    },
    {
      icon: () => (
        <img style={{ height: "40px", width: "40px" }} src={LinkIcon} />
      ),
    ...
    {
      icon: () => (
        <img style={{ height: "40px", width: "40px" }} src={RemoveIcon} />
      ),
      tooltip: "supprimer décideur",
      onClick: (event, rowData) => {
        setDecideurEmail(rowData.email);
  ...
  return decideursProfiles.loading || decideursProfiles.error ? null : (
    <div className="bg-white" style={{ height: 400, width: "100%" }}>
      ...
      <ProfileDialog
        ...
        setOpen={setProfileDialogOpenStatus}
        profileForm={selectedProfile}
        setProfileForm={setSelectedProfile}
      />
      <ConfirmDialog
        isOpen={isConfirmDialogopen}
        setOpen={setConfirmDialogOpenStatus}
        decideurEmail={decideurEmail}
      />
      <SignUpDialog
        isOpen={isSignUpDialogopen}
        setOpen={setSignUpDialogOpenStatus}
      />
    </div>
  );
}
