...
export default function LocatairesSignUpRequestsRoute() {
      ...
      render: (rowData) => (
        <div className="d-flex align-items-center">
          ...
          <div className="pl-2 flex-column">
            ...
            <div className="roboto-500 gacela-gray9E pt-2">
              {" "}
              {rowData.email}
            </div>
          </div>
    ...
    },
    {
      ...
      render: (rowData) => (
        <img
          className="hoverable"
          onClick={() =>
            setPermisDialog(`${baseUrlTest + "/" + rowData.photo_identity}`)
          }
    ...
    },
    {
      ...
      render: (rowData) => (
        <img
          className="hoverable"
          onClick={() =>
            setPhotoDialog(`${baseUrlTest + "/" + rowData.personal_photo}`)
          }
          style={{ height: "40px", width: "40px" }}
          src={IdentiteIcon}
        />
  ...
  const actions = [
    (rowData) => ({
      icon: () => (
        <img style={{ height: "40px", width: "40px" }} src={CheckIcon} />
      ),
      tooltip: "Confirmer l'inscription",
      onClick: (event, rowData) => {
        dispatch(fetchAcceptLocataire(rowData.email)).then(() => {
          setTimeout(() => {
    ...
    (rowData) => ({
      icon: () => (
        <img style={{ height: "40px", width: "40px" }} src={DeleteIcon} />
      ),
      tooltip: "Delete User",
      onClick: (event, rowData) => {
        setRejectDialogOpenStatus(true);
        setLocataireEmail(rowData.email);
      },
    }),
  ];

  return locatairesrequests.loading || locatairesrequests.error ? null : (
